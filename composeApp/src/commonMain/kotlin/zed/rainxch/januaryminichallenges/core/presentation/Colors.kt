package zed.rainxch.januaryminichallenges.core.presentation

import androidx.compose.ui.graphics.Brush
import androidx.compose.ui.graphics.Color

object WinterTravelGalleryColors {
    val bgMain = Brush.linearGradient(listOf(
        Color(0xffDAEEFF),
        Color(0xffEDF7FF),
    ))
    val bgGallery = Color(0xffFFFFFF)
    val bgError = Brush.linearGradient(listOf(
        Color(0xffFFE9EE),
        Color(0xffFFFBFC),
    ))
    val textPrimary = Color(0xff001221)
    val textCard = Color(0xffFFFFFF)
    val outlineBtn = Color(0xffC9D0D8)
    val primary = Color(0xff5799FC)
    val error = Color(0xffF7164B)
}

object SettingsColors {
    val bg = Color(0xffEDF2F8)
    val surface = Color(0xffFFFFFF)
    val surfaceInput = Color(0xffF5F8FA)
    val outline = Color(0xffC9D6E0)
    val outlineInput = Color(0xffE7EDF1)
    val textPrimary = Color(0xff101828)
    val textSecondary = Color(0xff3F4D63)
    val primary = Color(0xff50ADF5)
}

object RecipeColors {
    val bg = Color(0xffFFFDF9)
    val surfaceInput = Color(0xffDEDBD4).copy(alpha = .3f)
    val outline = Color(0xffDEDBD4)
    val overlay = Color(0xff080603).copy(alpha = .7f)
    val textPrimary = Color(0xff011A0E)
    val textSecondary = Color(0xff595447)
    val textOnPrimary = Color(0xffFFFFFF)
}